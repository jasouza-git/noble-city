...
export interface sprite2 {
    /**
     * What to fill in the sprite with, can be a color or image
     */
    fill?:string,
    /**
     * What to stroke the sprite with, must be color
     */
    stroke?:string,
    /**
     * A rectangle to box the sprite, must be 4 numbers
     * 1. X position
     * 2. Y position
     * 3. Width
     * 4. Height
    ...
    rect?:number[],
    /**
     * Render fixed to canvas, regardless of camera
     */
    fixed?:boolean,
    /**
     * Text to render at the origin
     */
    text?:string,
    /**
     * X position
    ...
    path?:number[][],
    /**
     * Point of Origin for position and rotation
     */
    axis?:''|'n'|'e'|'w'|'s'|'ne'|'nw'|'se'|'sw',
}
/**
 * ### Sprite Object
 * A simple data-structure to represent renderable objects
 ...
 * - `gx` - Dragged X distance
 * - `gy` - Dragged Y distance
 * - `grx` - Dragged X distance relative to canvas
 * - `gry` - Dragged Y distance relative to canvas
 * 
...
export class Entity {
    ...
    constructor(...x:any[]) { }
    update(...obj:sprite[]) {
        for (const o of obj) Object.assign(this.obj, o);
    }
...
export class Game {
    ...
    sprite(s:sprite) {
        ...
        if (s.d && s.d.includes('w')) { // Horizontal align to end
    ...
    render(func:(dt:number,t:number)=>void=()=>{}, init=true) {
        ...
        for (const entity of this.entity) {
            this.sprites(entity.sprite(dt/1000, d-this._start));
        }
